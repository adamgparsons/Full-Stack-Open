import React, { useState } from 'react';
import ReactDOM from 'react-dom';

const initScores = (anecdotes) =>
  new Array(anecdotes.length + 1).join('0').split('').map(parseFloat);

const Button = ({ text, handleClick }) => (
  <button onClick={handleClick}>{text}</button>
);

const generateRandom = () => {
  const randomQuote = Math.floor(Math.random() * anecdotes.length);
  return randomQuote;
};

const addVote = (selected, points) => {
  const updatedPoints = points;
  updatedPoints[selected] += 1;
  return updatedPoints;
};

const maxValue = (numArray) => Math.max(...numArray);

const indexOfMaxValue = (numArray) => numArray.indexOf(maxValue(numArray));

const App = (props) => {
  const [selected, setSelected] = useState('0');
  const [points, setPoints] = useState(initScores(anecdotes));
  const [topAnecdote, setTopAnecdote] = useState('null');

  const updatePoints = () => {
    setPoints(addVote(selected, points));
    setTopAnecdote(indexOfMaxValue(points));
  };

  const topAnnecdote = props.anecdotes[indexOfMaxValue(points)];
  return (
    <div>
      <h1>Quote generator</h1>
      <p>{props.anecdotes[selected]}</p>
      <Button handleClick={() => updatePoints()} text="vote" />
      <Button
        handleClick={() => setSelected(generateRandom())}
        text="Generate random quote"
      />
      <h2>Highest ranked anecdote</h2>
      <p>{props.anecdotes[topAnecdote]}</p>
      <p>With {points[topAnecdote]} points </p>
    </div>
  );
};

const anecdotes = [
  'If it hurts, do it more often',
  'Adding manpower to a late software project makes it later!',
  'The first 90 percent of the code accounts for the first 90 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
  'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
  'Premature optimization is the root of all evil.',
  'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.',
];

ReactDOM.render(<App anecdotes={anecdotes} />, document.getElementById('root'));
